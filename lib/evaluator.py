...
class MeshEvaluator:
    ...
    def _render_normal(self, mesh, deg):
        ...
        self._normal_render.set_normal_mesh(
            self.scale_factor * mesh.vertices,
            mesh.faces,
            mesh.vertex_normals,
            mesh.faces,
        )
    ...
    def get_reproj_normal_error(
        self, frontal=True, back=True, left=True, right=True, save_demo_img=None
    ):
        # reproj error
...
if __name__ == "__main__":
    ...
    def run(root, exp_name, tar_path):
                ...
                vals.append(
                    ...
                    * evaluator.get_reproj_normal_error(
                        save_demo_img=os.path.join(
                            src_path, "%s_%s.png" % (name, src[:-4])
                        )
                    )
                )

                item = {"name": "%s_%s" % (name, src), "vals": vals}
        ...
        vals = np.array(total_vals).mean(0)
        print(
            "BUFF - chamfer: %.4f  p2s: %.4f  nml: %.4f"
            % (buf_val[0], buf_val[1], buf_val[2])
        )
