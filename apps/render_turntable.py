...
parser.add_argument(
    "-g", "--geo_render", action="store_true", help="default is normal rendering"
)
...
for i, obj_path in enumerate(obj_files):
    ...
    for j in range(0, 361, 2):
        ...
        cam.eye = (
            np.array(
                [2.0 * math.sin(math.radians(j)), 0, 2.0 * math.cos(math.radians(j))]
            )
            + cam.center
        )
    ...
    cmd = (
        ...
        + obj_root
        + "/rot_%04d.png -vcodec libx264 -y -pix_fmt yuv420p -refs 16 "
        + os.path.join(obj_root, file_name + ".mp4")
    )
    os.system(cmd)
